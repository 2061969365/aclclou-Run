...
(async () => {
  ...
  try {
    ...
    console.log('[DOM] 页面所有 input:', JSON.stringify(inputsInfo, null, 2));

    // ── Step 2: 填邮箱
    // 用 placeholder 定位，因为截图中显示占位符是 "user@aclclouds.com"
    console.log('[2/5] 填写邮箱...');
    const emailInput = page.locator('input[placeholder*="aclclouds"], input[placeholder*="user"], input[placeholder*="Email"], input[placeholder*="email"]').first();
    await emailInput.waitFor({ state: 'visible', timeout: 30000 });
    await emailInput.fill(EMAIL);
    console.log('[2/5] 邮箱填写完成');

    // ── Step 3: 填密码 ──
    console.log('[3/5] 填写密码...');
    // 密码框用 type=password（通常不变）或紧跟在邮箱后的第二个 input
    const pwInput = page.locator('input[type="password"]').first();
    await pwInput.waitFor({ state: 'visible', timeout: 15000 });
    await pwInput.fill(PASSWORD);
    console.log('[3/5] 密码填写完成');

    // ── Step 4: 勾选人机验证 checkbox ──
    console.log('[4/5] 勾选人机验证...');
    // 用 label 文字定位更可靠
    const cb = page.locator('label:has-text("robot") input, input[type="checkbox"]').first();
    await cb.waitFor({ state: 'visible', timeout: 15000 });
    await cb.click();
    await page.waitForTimeout(1000);
    console.log('[4/5] 人机验证完成');
